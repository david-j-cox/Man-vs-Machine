...
def fit_gme(df, bx_cols, sr_cols, session_col):
    ...
    try:
        ...
        try:
            uniq_sessions = df_year[session_col].unique()
            if len(uniq_sessions) < 5:
                err_type.append('less than 5 total sessions')
            else:
                sesh_num = 1
                for k in uniq_sessions:
                    session_data = df_year[df_year[session_col ]== k]

                    if len(session_data) < 5:
                        err_type.append(f'not enough data for session {i} - leaving out of analysis')
                        pass
                    else:
                        # Behaviors
                        hard_straight_count.append(len(session_data.loc[session_data.pitch_name.isin(hrd_strt)]))
                        hard_breaking_count.append(len(session_data.loc[session_data.pitch_name.isin(hrd_brkng)]))
                        soft_straight_count.append(len(session_data.loc[session_data.pitch_name.isin(sft_strt)]))
                        soft_breaking_count.append(len(session_data.loc[session_data.pitch_name.isin(sft_brkng)]))
